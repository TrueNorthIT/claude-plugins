...
/**
 * SessionStart hook — checks the installed `create-gds-service` plugin
...
async function main() {
  ...
  try {
    const res = await fetch(
      "https://raw.githubusercontent.com/TrueNorthIT/claude-plugins/main/plugins/create-gds-service/.claude-plugin/plugin.json",
    );
    if (!res.ok) return;
  ...
  console.error(
    `\n  create-gds-service plugin v${localVersion} is outdated (latest: v${remoteVersion}).` +
