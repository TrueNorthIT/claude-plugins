...
/**
 * SessionStart hook — checks the installed `dataverse-terraform` plugin
...
async function main() {
  ...
  try {
    const res = await fetch(
      "https://raw.githubusercontent.com/TrueNorthIT/claude-plugins/main/plugins/dataverse-terraform/.claude-plugin/plugin.json",
    );
    if (!res.ok) return;
  ...
  console.error(
    `\n  dataverse-terraform plugin v${localVersion} is outdated (latest: v${remoteVersion}).` +
